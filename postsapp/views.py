...
class CreatePostView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            messages.error(
                request, "Error while creating post, check if all fields are filled"
            )
        return render(request, self.template_name, context)
...
def add_comment(request):
    if request.POST:
        ...
        comm = Comments.objects.create(
            author=request.user, text=request.POST.get("comment"), post=post
        )
        comm.save()
        data_response = {"success": True, "date": comm.date.strftime("%Y-%m-%d %H:%M")}
        return HttpResponse(
            json.dumps(data_response, cls=DjangoJSONEncoder),
            content_type="application/json",
        )
